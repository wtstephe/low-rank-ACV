...
class SyntheticDatasetGenerator(DatasetGenerator):
    ...
    def get_theta(self, D,
        ...
        if upTo is None:
          ...
          theta[:upTo] = np.random.normal(scale=sigma_theta,
                                          size=upTo)
    ...
    def get_X(self, N, D,
        ...
        Xrank is the rank of the covariate matrix; it can be an integer
          or a list of integers if you want the datapoints to be drawn
          from a union of subspaces.
        '''
        if Xrank is None:
...
class GisetteDatasetGenerator(DatasetGenerator):
  ...
  def get_dataset(self, filepath=None, **kwargs):
    ...
    dataPath = os.path.join(filepath, 'X-N=1000-D=2000.txt')
    labelsPath = os.path.join(filepath, 'Y-N=1000-D=2000.txt')
...
class P53DatasetGenerator(DatasetGenerator):
  ...
  def __init__(self, filepath=None):
    if filepath is None:
      self.filepath = '../data/p53_new_2012'
    else:
      self.filepath = filepath
    super(DatasetGenerator, self).__init__()
...
class RCV1DatasetGenerator(DatasetGenerator):
  ...
  def get_dataset(self, filepath=None, **kwargs):
    if filepath is None:
        filepath = '../data/rcv1'
    dataPath = os.path.join(filepath, 'X-N=20000-D=20000.txt')
    labelsPath = os.path.join(filepath, 'Y-N=20000-D=20000.txt')
...
class BlogFeedbackDatasetGenerator(DatasetGenerator):
  '''
  From https://archive.ics.uci.edu/ml/datasets/BlogFeedback
  '''
  def __init__(self):
    super(DatasetGenerator, self).__init__()
  ...
  def get_dataset(self, filepath=None, **kwargs):
    ...
    goodYs = np.setdiff1d(np.arange(X.shape[0]),
                          np.where(Y > 1000)[0])
    X = X[goodYs]
    Y = Y[goodYs]
