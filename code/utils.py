...
def evalLOO(model, LOOParams):
  loss = 0.0
  dataset = copy.deepcopy(model.training_data)
  for nn in range(dataset.X.shape[0]):
    dataset.X = model.training_data.X[nn].reshape(1,-1)
    dataset.Y = np.array(model.training_data.Y[nn])
    loss += model.get_error(dataset)
  return loss / model.training_data.X.shape[0]


def logisticLoss(iprods, Y):
  paramsXY = -Y * iprods
  M = np.maximum(paramsXY, 0)
  log_likelihood = -(np.log(np.exp(0-M) + np.exp(paramsXY-M)) + M)
  return -log_likelihood.sum()

def poissonLoss(iprods, Y):
  M = np.maximum(iprods, 0.0)
  lam = np.log(np.exp(0-M) + np.exp(iprods-M)) + M
