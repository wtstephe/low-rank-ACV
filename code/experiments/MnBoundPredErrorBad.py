...
IJAssUpperBndCorrected = np.zeros(nIter)
...
for ii in range(nIter):
  ...
  # Replace top 2 highest bounds with exact CV
  maxInds = np.argsort(predErrsIJUpperBnd)[-2:]
  predErrsIJUpperBnd[maxInds] = predErrsExact[maxInds]
  IJAssUpperBndCorrected[ii] = predErrsIJUpperBnd.mean()
...
plt.title('Avg. exact CV vs avg. bounds across trials',
          fontsize=titleFontsize)
plt.tight_layout()
plt.savefig('C:YOUR_FILEPATH/averageTrialErrorBounds-Poisson-IJ-badTrials.png', bbox='tight')


plt.figure()
errBars = np.vstack([IJAssLowerBnd[sortInds],
                     IJAssUpperBndCorrected[sortInds]])
plt.scatter(xs[:upTo], exactAss[sortInds][:upTo],
            label='Exact CV',
            c='b')
plt.errorbar(xs[:upTo]+jitter, IJAss[sortInds][:upTo],
...
plt.title('Avg. exact CV vs avg. bounds across trials',
          fontsize=titleFontsize)
plt.tight_layout()
plt.savefig('C:YOUR_FILEPATH/averageTrialErrorBounds-Poisson-IJ-badTrialsCorrected.png', bbox='tight')
plt.show()
