...
def leave_k_out_cv(model, k, method="IJ",
  ...
  if hold_outs != "exact":
    ...
    held_out_sets = [hos for hos in held_out_sets]
  count = 0
  total_error = 0
  num_sets = 0
  ...
  if method == 'IJ':
    model.compute_dParams_dWeights(np.ones(N), non_fixed_dims=non_fixed_dims,
                                   **kwargs)
  elif method == 'NS':
  ...
  for i, held_out_set in enumerate(held_out_sets):
    ...
    if method=="IJ":
      ...
      print('\n\n I dont recognize this method %s in leave_k_out_cv\n\n',
            method)
  ...
  return total_error * 1.0 / count, all_params, held_out_sets
...
def leave_k_out_cv_parallel(model, k, method="IJ",
  ...
  def run_me(held_out_set, model, approx, runSlowAppxCV,
  ...
  return np.array(cv_errors).mean(), np.array(params), held_out_sets
